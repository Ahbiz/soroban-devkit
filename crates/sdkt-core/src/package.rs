//! Local package-manager foundation for the Soroban DevKit (sdkt), .
//!
...
/// returned `Option`.
///
/// Selection rule (): among the remote tags whose names parse as valid
...
// `verify_dependencies` + `compute_dependency_integrity` () — no new
...
/// A self-contained, offline package bundle descriptor ().
///
...
/// Result of a `publish --dry-run` readiness check ().
///
...
/// Bundle a project into a portable offline artifact ().
///
...
/// Evaluate publish readiness for a project ( `publish --dry-run`).
///
/// Pure read-only validation built on [`crate::lock::verify_dependencies`]
/// () plus manifest/lock presence. Detects: missing cache, lock drift,
